...
## 4. API Endpoints and Annotation Logic
...
4.  **Private Check**: If the annotation is marked `isPrivate`, it is visible *only* to the creator and Admin.
5.  **Shared Check**: If the annotation is *not* private, it is visible if the `currentUser`'s role is explicitly included in the annotation's `visibility` array.
